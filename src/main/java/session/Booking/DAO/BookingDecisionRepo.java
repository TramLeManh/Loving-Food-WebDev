...
public interface BookingDecisionRepo extends JpaRepository<BookingDecision, Integer> {
    ...
    @Query("SELECT b FROM BookingDecision b WHERE b.adminUserId = :id")
    List<BookingDecision> viewAllBookingDecision(int id);
    @Query("SELECT b FROM BookingDecision b WHERE b.bookingInformation.bookingId = :booking_id")
    BookingDecision viewBookingDecisionDetailByBookingId(int booking_id);
    @Query("SELECT b FROM BookingDecision b WHERE b.decision_id = :decision_id")
    BookingDecision viewBookingDecisionDetailByDecisionId(int decision_id);
    @Modifying
    @Transactional
    ...
    @Query("SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM BookingDecision b WHERE b.bookingInformation.bookingId = :id")
    Boolean isDecisionExist(@Param("id") int id);
    @Query("SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM BookingDecision b WHERE b.bookingInformation.bookingId = :id AND b.status = 'POSTPONED'")
